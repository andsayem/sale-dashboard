...
class RemoteServices {
  ...
  static Future<List<String>> fetchDivisions() async {
    var response = await client.get(Uri.parse(
        'https://dashboard.ssgbd.com/app_api/get_division.php?username=dashboard'));
    if (response.statusCode == 200) {
      var jsonString = response.body;
      print('Test----------------');
  ...
  static Future<List<String>> fetchcChannels() async {
    var response = await client.get(Uri.parse(
        'https://dashboard.ssgbd.com/app_api/get_channel.php?username=dashboard'));
    if (response.statusCode == 200) {
      var jsonString = response.body;
  ...
  static Future<Login> fetchcLogin(password) async {
    var response = await client.get(Uri.parse(
        'https://dashboard.ssgbd.com/app_api/login.php?username=dashboard&password=' +
            password));
    if (response.statusCode == 200) {
      var jsonString = response.body;
      print(jsonString);
